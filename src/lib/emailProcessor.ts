...
async function processAsNewComment(emailMessage: Message, ticketId: number, state: EmailProcessingState): Promise<ProcessEmailResult> {
    const logAndReturn = (result: ProcessEmailResult, logMessage: string) => { /* ... */ return result; };
    const bodyContent = nullableToOptional(emailMessage.body?.content) || '';
    ...
    await db.update(tickets).set({ status: OPEN_STATUS, updatedAt: new Date() }).where(eq(tickets.id, ticketId));
    await graphService.markEmailAsRead(state.messageId!);

    ticketEventEmitter.emit({ type: 'comment_added', ticketId: ticketId });

    return { success: true, commentId: 1, message: `Comment added to ticket ${ticketId}` }; // Returning dummy ID for now
}
