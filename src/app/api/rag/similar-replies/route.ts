...
async function handle(request: NextRequest, payload: unknown) {
  ...
  try {
    const results = await ragRepository.findSimilarReplies({
    ...
    console.error('[rag.similar-replies] Failed to load results:', err);
    return apiError('server_error', 'Failed to load similar replies', undefined, { status: 500 });
  }
}
