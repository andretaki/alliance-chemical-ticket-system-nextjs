  ...
  const content = (
    <Card className="group relative overflow-hidden border-white/[0.06] bg-white/[0.02] transition-all hover:border-white/[0.1] hover:bg-white/[0.04]">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <div className={cn('rounded-lg bg-white/[0.04] p-2', variantStyles[variant])}>
...
function TicketRow({ ticket }: { ticket: TicketSummary }) {
  ...
  return (
    <Link
      ...
      className="group flex items-center gap-4 rounded-lg border border-transparent px-3 py-3 transition-all hover:border-white/[0.06] hover:bg-white/[0.02]"
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="flex-shrink-0 text-xs text-white/30">#{ticket.id}</span>
        </div>
  ...
  return (
    <Link href={href}>
      <Button
        ...
        className={cn(
          'h-auto flex-col gap-2 px-4 py-4',
          variant === 'primary'
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
            ...
            <Card className="border-white/[0.06] bg-white/[0.02]">
              <CardHeader className="border-b border-white/[0.06] pb-4">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-medium text-white">Tickets</CardTitle>
                  <Link
                    href="/tickets"
                ...
                <div className="border-b border-white/[0.06] px-4">
                  <TabsList className="h-10 w-full justify-start gap-4 bg-transparent p-0">
                    <TabsTrigger
                      ...
                      className="relative h-10 rounded-none border-b-2 border-transparent bg-transparent px-0 pb-3 pt-2 text-sm font-medium text-white/50 transition-all data-[state=active]:border-indigo-500 data-[state=active]:text-white"
                    >
                      Needs Attention
                      {urgentTickets.length > 0 && (
                        <Badge
                          variant="outline"
                          className="ml-2 h-5 border-red-500/30 bg-red-500/15 px-1.5 text-[10px] text-red-400"
                        >
                    ...
                    >
                      ...
                      {newTickets.length > 0 && (
                        <Badge
                          variant="outline"
                          className="ml-2 h-5 border-indigo-500/30 bg-indigo-500/15 px-1.5 text-[10px] text-indigo-400"
                        >
                          {newTickets.length}
                        </Badge>
                      )}
                    ...
                    <div className="p-2">
                      ...
                      <TabsContent value="new" className="m-0">
                        {isLoading ? (
                          ...
                          <div className="flex flex-col items-center justify-center py-12 text-center">
                            <Ticket className="mb-3 h-10 w-10 text-white/20" />
                            <p className="text-sm font-medium text-white/70">No new tickets</p>
                            <p className="mt-1 text-xs text-white/40">
                      ...
                      <TabsContent value="recent" className="m-0">
                        {isLoading ? (
                          ...
                          <div className="flex flex-col items-center justify-center py-12 text-center">
                            <Activity className="mb-3 h-10 w-10 text-white/20" />
          ...
          <div className="space-y-6">
            ...
            {/* AI Insights */}
            <Card className="border-white/[0.06] bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-transparent">
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center gap-2 text-base font-medium text-white">
              ...
              <CardContent className="space-y-3">
                <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
                  <div className="mb-1 flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-white/80">Response Time</span>
                  </div>
                ...
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
                  <div className="mb-1 flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-white/80">Attention Needed</span>
                  </div>
                  <p className="text-xs text-white/50">
            ...
            <Card className="border-white/[0.06] bg-white/[0.02]">
              <CardHeader className="pb-4">
                <CardTitle className="text-base font-medium text-white">Team Activity</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-xs text-white/70">
                          <span className="font-medium text-white/90">{activity.name}</span>{' '}
                          {activity.action}{' '}
                          <span className="text-indigo-400">{activity.ticket}</span>
                        </p>
                        <p className="mt-0.5 text-[10px] text-white/30">{activity.time}</p>
                      </div>
                    </div>
                  ))}
                </div>
